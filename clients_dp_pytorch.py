...
def train(net, trainloader, optimizer,privacy_engine, epochs):
    ...
    for _ in range(epochs):
        for i, (inputs, target) in enumerate(trainloader):
            ...
            return round(epsilon, 2), round(np.mean(losses), 2)
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def __init__(self, number):
        ...
        self.optimizer = SGD(self.net.parameters(), lr=0.001, momentum=0.9)

        # enter PrivacyEngine
        self.privacy_engine = PrivacyEngine()
        self.net, self.optimizer, self.train_dl = self.privacy_engine.make_private_with_epsilon(
            module=self.net,
